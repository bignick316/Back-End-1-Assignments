...
if ($action == 'list_vehicles') {
    ...
    include('C:\xampp\htdocs\Week 2\Midterm\View\admin\admin_vehicle_list.php');
}
else if($action == 'show_insert_vehicle' || $action == 'delete_vehicle' || $action == 'insert_vehicle') {
    $types = type_db::get_types();
    $makes = make_db::get_makes();
    $classes = class_db::get_classes();
    include('controllers/vehicle.php');
} 
else if($action == 'insert_type' || $action == 'delete_type' || $action == 'edit_type') {
    $types = type_db::get_types();
    include('controllers/type.php');
} 
else if($action == 'insert_make' || $action == 'delete_make' || $action == 'edit_make') {
    $makes = make_db::get_makes();
    include('controllers/make.php');
} 
else if($action == 'insert_class' || $action == 'delete_class' || $action == 'edit_class') {
    $classes = class_db::get_classes();
