...
const createNewFunfact = async (req, res) =>
{
    ...
    {
        res.status(400).json({"message": "State fun facts value needed"});
    }
    else if (!Array.isArray(req.body.funfacts))
    {
        res.status(400).json({"message": "State fun facts value have to be in an array"})
    }
    else
    {
...
const updateFunfact = async (req, res) => {
    ...
    {
        res.status(400);
        res.json({'message': 'State fun fact index value needed'});
    }
    else if (!req?.body?.funfact)
    {
        res.status(400);
        res.json({'message': 'State fun fact value needed'});
    }
    else
    {
...
const deleteFunfact = async (req, res) => {
    if (!req?.body?.index)
    {
        res.status(400).json({"message": 'State fun fact index value needed'});
    }
    else
    {
        var state = statesJson.find((state) => state.code == req.params.state.toUpperCase());
